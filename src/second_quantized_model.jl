...
- Fields:
    ...
    - `particle_statistics::Particle_Statistics`: the statistics of the particle (or the model), i.e., `Bosonic()` or `Fermionic`
...
mutable struct Real_Space_Second_Quantized_Model{T} <: Second_Quantized_Model
    ...
    bilinear_terms::Vector{Tuple{Int,Int,T}} # `b†_i b_j` or `c†_i c_j` depending on particle_statistics
...
The model's `bilinear_terms` field is replaced; `params[\"twisted_phases_over_2π\"]`
